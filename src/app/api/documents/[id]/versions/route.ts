...
// History is append-only — old versions are never deleted.
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Verify ownership before returning any version data — per S1-003 §4.3.
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Verify ownership before any write — per S1-003 §4.3.
  // Non-owners get 404, never 403 — per S1-003 §5.5.
  const { data: doc, error: docError } = await supabase
    .from('documents')
    .select('id, content')
  ...
  // Step 1: Append a new version entry — history is never deleted.
  ...
  // Step 2: Update the document's main content so it reflects the
  // restored/new version immediately when loaded.

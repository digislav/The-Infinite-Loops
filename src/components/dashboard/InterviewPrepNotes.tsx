...
export function InterviewPrepNotes({ jobId, initialNotes, onSaved }: InterviewPrepNotesProps) {
  ...
  // Shared textarea style reused across all fields.
  const textareaClass =
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* EDIT MODE — five structured fields */}
      {isEditing ? (
        <div className="flex flex-col gap-4">
          ...
          {/* Other */}
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="prep-other" className="text-xs font-semibold text-gray-600">
              Other
            </Label>
            <p className="text-xs text-gray-400">Anything else that doesn&apos;t fit above.</p>
            <textarea
              id="prep-other"
              value={draft.other}
              onChange={(e) => handleFieldChange('other', e.target.value)}
              placeholder="e.g. Follow up with recruiter by Friday if no response..."
              className={textareaClass}
            />
          </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          {notes.other && (
            <div className="flex flex-col gap-1">
              ...
              <p className="text-sm whitespace-pre-wrap text-gray-600">{notes.other}</p>
            </div>
          )}
        </div>
      )}
